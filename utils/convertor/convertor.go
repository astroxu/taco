...
func ToBytes(value any) ([]byte, error) {
	...
	case float32:
		...
		bytess := make([]byte, 4) //collides
		binary.BigEndian.PutUint32(bytess, bits)
		return bytess, nil
	case float64:
		number := v.Float()
		bits := math.Float64bits(number)
		bytess := make([]byte, 8) //collides
		binary.BigEndian.PutUint64(bytess, bits)
		return bytess, nil
	case bool:
...
// StructToMap convert struct to mao,only convert exported struct field
...
func StructToMap(value any) (map[string]any, error) {
	...
	if t.Kind() != reflect.Struct {
		return nil, fmt.Errorf("data type %T not support,should be struct or pointer to struct", value)
	}
