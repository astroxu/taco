...
//InsertionSort user insertion to sort slice.
...
func QuickSort[T any](slice []T, lowIndex int, highIndex int, comparator constraints.Comparator) {
...
func partition[T any](slice []T, lowIndex int, highIndex int, comparator constraints.Comparator) int {
	p := slice[highIndex]
	i := lowIndex
...
// HeapSort user heap to sort slice
...
func sift[T any](slice []T, lowIndex int, highIndex int, comparator constraints.Comparator) {
	i := lowIndex
	j := 2*i + 1

	temp := slice[i]
	for j <= highIndex {
		if j < highIndex && comparator.Compare(slice[j], slice[j+1]) == -1 { // slice[j] < s;ice[j+1]
			j++
		}
...
func mergeSort[T any](slice []T, lowIndex int, highIndex int, comparator constraints.Comparator) {
	if lowIndex < highIndex {
